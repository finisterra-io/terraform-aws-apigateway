...
| <a name="input_gateway_responses"></a> [gateway\_responses](#input\_gateway\_responses) | A map of gateway response objects keyed by response type | <pre>map(object({<br>    status_code : string<br>    response_templates : optional(map(string))<br>    response_parameters : optional(map(string))<br>  }))</pre> | `{}` | no |
